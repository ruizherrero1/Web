...
const SHORT_CACHE_MS = 90_000;
const LONG_CACHE_MS = 30 * 60_000;
...
function normalizeScore(match: FootballDataMatch): AppScore | undefined {
...
function normalizeFootballDataMatch(match: FootballDataMatch): AppMatch {
...
function matchKey(match: AppMatch) {
  const teams = [match.team1, match.team2].sort().join("|");
  return `${match.date}|${teams}`;
}

function mergeMatches(baseMatches: AppMatch[], liveMatches: AppMatch[]) {
...
function hasMatchToday(matches: AppMatch[]) {
  const today = new Date().toISOString().slice(0, 10);
  return matches.some((match) => match.date === today);
}

function cacheDuration(matches: AppMatch[]) {
  return hasMatchToday(matches) ? SHORT_CACHE_MS : LONG_CACHE_MS;
}

async function fetchOpenFootball(): Promise<AppTournamentData> {
  ...
  const data = (await response.json()) as AppTournamentData;
  return {
    ...data,
    source: "openfootball",
...
async function fetchFootballData(): Promise<AppMatch[]> {
...
async function buildTournamentData(): Promise<AppTournamentData> {
  let baseData: AppTournamentData | null = null;
...
export async function GET() {
  ...
  const data = await buildTournamentData();
  cachedData = {
    data,
    expiresAt: now + cacheDuration(data.matches),
  };
